...
import {Action, ActionInfo, RelativeQuorumConfig} from "src/lib/Structs.sol";
...
contract RelativeQuorum is ILlamaStrategy, Initializable {
  ...
  function initialize(bytes memory config) external initializer {
    RelativeQuorumConfig memory strategyConfig = abi.decode(config, (RelativeQuorumConfig));
