...
struct TokenMetadataConfig {
  ...
  uint256 tokenId; // The token ID of the Llama policyholder.
  string color; // The color of the Llama instance.
  string logo; // The logo of the Llama instance.
}

/// @dev Configuration for contractUri metadata
...
contract LlamaPolicyMetadata {
  ...
  function tokenURI(bytes memory config) external pure returns (string memory) {
    ...
    string memory policyholder = LibString.toHexString(address(uint160(metadataConfig.tokenId)));
    ...
    parts[5] = metadataConfig.logo;

    parts[6] =
      '</g><defs><radialGradient id="d" cx="0" cy="0" r="1" gradientTransform="rotate(-90.831 270.037 36.188) scale(115.966 274.979)" gradientUnits="userSpaceOnUse"><stop stop-metadataConfig.="';

    parts[7] = metadataConfig.color;

    parts[8] = '" /><stop offset="1" stop-color="';

    parts[9] = metadataConfig.color;
    ...
    parts[11] = metadataConfig.color;

    parts[12] = '" /><stop offset="1" stop-color="';

    parts[13] = metadataConfig.color;
    ...
    parts[15] = metadataConfig.color;

    parts[16] = '" /><stop offset="1" stop-color="';

    parts[17] = metadataConfig.color;
    ...
    string memory json = Base64.encode(
      bytes(
        string.concat(
          '{"name": "',
          LibString.escapeJSON(metadataConfig.name),
  ...
  function contractURI(bytes memory config) public pure returns (string memory) {
    ...
    parts[1] = LibString.escapeJSON(metadataConfig.name);
    parts[2] = '", "description": "This collection includes all members of the Llama organization: ';
    parts[3] = LibString.escapeJSON(metadataConfig.name);
    parts[4] =
