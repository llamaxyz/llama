...
contract VertexFactory {
  ...
  VertexPolicyToken public vertexPolicyToken;
  ...
  function tokenURI(string memory name, string memory symbol, uint256 tokenId) external view returns (string memory) {
    ...
    return vertexPolicyToken.tokenURI(name, symbol, tokenId, color, logo);
  }

  /// @notice Sets the Vertex Policy Metadata contract.
  /// @param _vertexPolicyToken The Vertex Policy Metadata contract.
  function setPolicyMetadata(VertexPolicyToken _vertexPolicyToken) external onlyRootVertex {
    vertexPolicyToken = _vertexPolicyToken;
  }
