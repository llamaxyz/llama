...
contract IsActionCancelationValid is VertexStrategyTest {
  function testFuzz_ReturnsTrueForDisapprovedActions(uint256 _actionDisapprovals) {
    // TODO
    // call isActionCancelationValid on an action that has sufficient (random)
    // num of disapprovals. assert response is true
  }

  function testFuzz_ReturnsFalseForActionsNotFullyDisapproved(uint256 _actionApprovals) {
    // TODO
...
contract GetApprovalWeightAt is VertexStrategyTest {
  funcion testFuzz_ReturnsZeroWeightPriorToAccountGainingPermission(
  ...
  funcion testFuzz_ReturnsWeightAfterBlockThatAccountGainedPermission(
  ...
  function testFuzz_ReturnsZeroWeightForNonPolicyHolders(
