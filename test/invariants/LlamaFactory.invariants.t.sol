...
import {LlamaInstanceConfig, LlamaPolicyConfig, RoleHolderData, RolePermissionData} from "src/lib/Structs.sol";
...
contract LlamaFactoryHandler is BaseHandler {
  ...
  function llamaFactory_deploy() public recordCall("llamaFactory_deploy") useCurrentTimestamp {
    ...
    LlamaInstanceConfig memory instanceConfig = LlamaInstanceConfig(
      name(), relativeHolderQuorumLogic, accountLogic, encodedStrategyConfig, new bytes[](0), policyConfig
    );

    LLAMA_FACTORY.deploy(instanceConfig);
  }
}
