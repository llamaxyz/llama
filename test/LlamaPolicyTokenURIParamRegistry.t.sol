...
contract LlamaPolicyTokenURIParamRegistryTest is LlamaTestSetup {
  event ColorSet(LlamaCore indexed llamaCore, string color);
  event LogoSet(LlamaCore indexed llamaCore, string logo);
}

contract Constructor is LlamaPolicyTokenURIParamRegistryTest {
  function test_SetRootLlama() public {
    assertEq(address(policyTokenURIParamRegistry.ROOT_LLAMA()), address(rootCore));
  }
}

contract GetMetadata is LlamaPolicyTokenURIParamRegistryTest {
...
contract SetColor is LlamaPolicyTokenURIParamRegistryTest {
  ...
  function testFuzz_RevertIf_CallerIsNotLlama(address caller, LlamaCore llamaCore, string memory color) public {
    ...
    vm.expectRevert(LlamaPolicyTokenURIParamRegistry.OnlyLlama.selector);
    vm.prank(caller);
    policyTokenURIParamRegistry.setColor(llamaCore, color);
  }
}

contract SetLogo is LlamaPolicyTokenURIParamRegistryTest {
  ...
  function testFuzz_RevertIf_CallerIsNotLlama(address caller, LlamaCore llamaCore, string memory logo) public {
    ...
    vm.expectRevert(LlamaPolicyTokenURIParamRegistry.OnlyLlama.selector);
    vm.prank(caller);
    policyTokenURIParamRegistry.setLogo(llamaCore, logo);
  }
}
