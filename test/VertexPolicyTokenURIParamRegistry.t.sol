...
contract Constructor is VertexPolicyTokenURIParamRegistryTest {
  function test_SetRootVertex() public {
    assertEq(address(policyTokenURIParamRegistry.ROOT_VERTEX()), address(rootCore));
  }
}

contract GetMetadata is VertexPolicyTokenURIParamRegistryTest {
...
contract SetColor is VertexPolicyTokenURIParamRegistryTest {
  function test_SetsColor_CallerIsRootVertex() public {
    ...
    policyTokenURIParamRegistry.setColor(mpCore, color);
    assertEq(policyTokenURIParamRegistry.color(mpCore), color);
  }
  ...
  function testFuzz_RevertIf_CallerIsNotVertex(address caller, VertexCore vertexCore, string memory color) public {
    ...
    vm.expectRevert(VertexPolicyTokenURIParamRegistry.OnlyVertex.selector);
    vm.prank(caller);
    policyTokenURIParamRegistry.setColor(vertexCore, color);
  }
}

contract SetLogo is VertexPolicyTokenURIParamRegistryTest {
  function test_SetsLogo_CallerIsRootVertex() public {
    ...
    policyTokenURIParamRegistry.setLogo(mpCore, logo);
    assertEq(policyTokenURIParamRegistry.logo(mpCore), logo);
  }
  ...
  function testFuzz_RevertIf_CallerIsNotVertex(address caller, VertexCore vertexCore, string memory logo) public {
    ...
    vm.expectRevert(VertexPolicyTokenURIParamRegistry.OnlyVertex.selector);
    vm.prank(caller);
    policyTokenURIParamRegistry.setLogo(vertexCore, logo);
  }
}
