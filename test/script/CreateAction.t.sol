...
contract Run is CreateActionTest {
  ...
  function test_actionCanBeExecuted() public {
    ...
    PercentageQuorum[] memory strategiesAuthorized = new PercentageQuorum[](3);
    ...
    for (uint256 i = 0; i < emittedEvents.length; i++) {
      ...
      if (eventSig == strategiesAuthorizedSig) {
        ...
        strategiesAuthorized[strategiesCount++] = PercentageQuorum(strategy);
      }
    ...
    PercentageQuorum firstStrategy = strategiesAuthorized[0];
    ...
    PercentageQuorum secondStrategy = strategiesAuthorized[1];
    ...
    PercentageQuorum thirdStrategy = strategiesAuthorized[2];
